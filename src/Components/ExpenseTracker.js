  ...
  const addIncome = async () => {
    ...
    if (!isNaN(amount) && isFinite(amount)) {
      if (amount >= 0 && user) {
        ...
        setNewIncome({
          amount: '', // Clear the input field after adding income
        });
      } else {
...
endOfWeek.setHours(23, 59, 59, 999); // Set to the last millisecond of the day
  ...
  <button onClick={addIncome}>{isEditing ? 'Update Income' : 'Add Income'}</button>
</div>
    ...
    <div>
      <label>Cost:</label>
      <input type="text" value={newExpense.cost} onChange={(e) => setNewExpense({ ...newExpense, cost: e.target.value })} />
    </div>
    <div>
      ...
      {newExpense.category === 'other' && (
        <div>
          ...
          />
          <button onClick={() => addNewCategory(newExpense.newCategory)}>Add Category</button>
        </div>
      )}
    </div>
    <div>
    ...
    {editingExpenseId && (
      <button onClick={cancelEdit} style={{ marginLeft: '10px' }}>Cancel Edit</button>
    )}
    <div>
